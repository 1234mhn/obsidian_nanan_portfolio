...
## 两大流派核心对比（2026.7.30 讨论）

| | Bagging（随机森林） | Boosting（XGBoost） |
|:--|:-----------------|:------------------|
| 训练方式 | **并行**，互不影响 | **串行**，后一个补前一个的错 |
| 类比 | 一组专家各自独立投票 | 一个团队接力改进 |
| 出错方式 | 各错各的，一投票就抵消了 | 集中火力攻克最难的点 |
| 过拟合风险 | 低 ✅ | 高（太专注错题了） |
| 精度天花板 | 不错，但有限 | 能到很高 🏆 |
| 训练速度 | 快（并行） | 慢（串行） |
...
| 场景 | 推荐 |
|:----|:----|
| 数据干净、特征好 | 随机森林（快、稳、不易过拟合） |
| 竞赛冲榜 | XGBoost / LightGBM（精度高，但要调参） |
| 超大数据（百万级） | XGBoost GPU加速版 |
